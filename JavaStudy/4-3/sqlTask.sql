...
-- [回答]
select store_table.store_name,goods_table.goods_name,stock_table.quantily
from stock_table
    join store_table on stock_table.quantily = store_table.store_name 
...
-- [回答]
select store_code,平均値 from (select store_code,avg(quantily)as 平均値 from store_table by store_code)as nes
    where 平均値 >= avg quantily,store_code='EA01'
...
values ('M001','マフラー','4500円','sysdate')

-- 7. 在庫テーブルの商品コード='S987'、かつ、店舗コード='EA01'に対して、「在庫数=10、更新日付=本日日付」で更新しなさい。※実行後のSELECT結果も貼付すること。
-- [回答]
update store_table set quantily=10,update_day=sysdate
where goods_code='S987',store_code='EA01'

-- 8. 7で更新した商品を削除しなさい。※実行後のSELECT結果も貼付すること。
-- [回答]
delete from store_table where goods_code='S987'and store_code='EA01'

update store_table set goods_code='S987',store_code='EA01'